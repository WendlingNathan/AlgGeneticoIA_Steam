...
function render(){
  ...
  $('resultNote').textContent=`${fmt(last.selected.length,0)} jogos pagos. ${config.useDisk?`Espaço: ${fmt(best.size)} de ${fmt(config.disk)} GB. `:''}Pesos: satisfação ${config.weights.satisfaction}, avaliações ${config.weights.positive}, horas ${config.weights.playtime}. Total: soma das notas de 0 a 10 por jogo. Semente ${config.seed}.`;
...
function updateGames(){if(!last)return;const query=$('search').value.trim().toLocaleLowerCase('pt-BR');visible=last.selected.filter(i=>rows[i][1].toLocaleLowerCase('pt-BR').includes(query)).sort((a,b)=>rows[b][2]-rows[a][2]||rows[a][1].localeCompare(rows[b][1]));page=0;renderPage();}
