...
function render(){
  ...
  const s=last.stats;
  $('filterReport').textContent=`Catálogo: ${fmt(s.total,0)}. Elegíveis: ${fmt(last.candidates,0)}. Exclusões, contadas uma vez por registro: tags bloqueadas ${fmt(s.blockedTags,0)}; dados básicos inválidos ${fmt(s.invalid,0)}; duplicados ${fmt(s.duplicates,0)}; menos de 100 avaliações ou total desconhecido ${fmt(s.insufficientReviews,0)}; fora das características escolhidas ${fmt(s.characteristicsMismatch,0)}; métrica ausente ${fmt(s.missingMetric,0)}; tamanho desconhecido ${fmt(s.missingDisk,0)}; gratuitos desativados ${fmt(s.freeExcluded,0)}; preço acima do limite ${fmt(s.tooExpensive,0)}; tamanho acima do limite ${fmt(s.tooLarge,0)}; valor zero ${fmt(s.zeroValue,0)}.`;
  $('evolutionText').textContent=`Geração 0: ${fmt(last.history[0].best)} → geração ${config.generations}: ${fmt(best.value)}. ${fmt(last.fixedCount,0)} jogos sem custo nos recursos limitados entram automaticamente. População: ${config.population}; mutação por gene variável: ${fmt(last.mutationRate*100,5)}%.`;
}
function updateGames(){if(!last)return;const query=$('search').value.trim().toLocaleLowerCase('pt-BR'),type=$('showType').value;visible=last.selected.filter(i=>rows[i][1].toLocaleLowerCase('pt-BR').includes(query)&&(type==='all'||(type==='free'?rows[i][2]===0:rows[i][2]>0))).sort((a,b)=>rows[b][2]-rows[a][2]||rows[a][1].localeCompare(rows[b][1]));page=0;renderPage();}
