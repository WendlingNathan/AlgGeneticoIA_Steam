...
(function(root) {
  ...
  function createOptimizer(rows, config) {
    ...
    for(let index=0;index<rows.length;index++){
      ...
      let value;
      if(config.objective==='positive')value=finite(r[3])?r[3]:null;
      else if(config.objective==='playtime')value=finite(r[5])?r[5]/60:null;
      else value=finite(r[3])&&finite(r[4])&&r[3]+r[4]>0?100*r[3]/(r[3]+r[4]):null;
      if(value===null){stats.missingMetric++;continue;}
      ...
      const game={index,price:r[2],size,value};
    ...
    function result(){const best=population[0];return {config,stats,candidates:n+fixed.length,variableGenes:n,fixedCount:fixed.length,mutationRate,history,selected:fixed.concat(best.genes.map(i=>eligible[i].index)),best:{value:best.value,cost:best.cost,size:best.size,feasible:best.feasible},greedy:{value:greedy.value,cost:greedy.cost,size:greedy.size},random:{value:randomBest.value,cost:randomBest.cost,size:randomBest.size}};}
    return {step,result};
  }
