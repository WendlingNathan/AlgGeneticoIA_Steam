...
for(const maxGames of [1,2,10])for(const limits of [{useBudget:true,useDisk:false},{useBudget:false,useDisk:true},{useBudget:true,useDisk:true}]){const result=run(fixture,{...base,...limits,maxGames,flips:100,crossover:1});verify(result,fixture);assert.deepEqual(result,run(fixture,{...base,...limits,maxGames,flips:100,crossover:1}));}
const single=run(fixture,{...base,maxGames:1});assert.deepEqual(single.selected,[2]);assert.equal(single.best.value,10);
for(const maxGames of [0,-1,1.5,NaN,Infinity,null,'3'])assert.throws(()=>run(fixture,{...base,maxGames}),/quantidade de jogos/);
const few=run(fixture.slice(0,1),{...base,maxGames:10});assert.equal(few.selected.length,1);
const zeroSize=fixture.map(r=>{const row=r.slice();row[7]=0;return row;});verify(run(zeroSize,{...base,useBudget:false,useDisk:true,maxGames:1}),zeroSize);
