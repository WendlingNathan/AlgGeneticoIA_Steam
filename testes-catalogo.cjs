...
const base={budget:500,useBudget:true,disk:10,useDisk:false,includeFree:true,weights:{satisfaction:0,positive:10,playtime:0},population:30,generations:80,flips:2,crossover:.8,seed:42};
...
const tiny=run(fixture,base);assert(Math.abs(tiny.best.value-160/9)<1e-10);verify(tiny,fixture);assert.deepEqual(tiny,run(fixture,base));
...
const unknownHours=[['x','Sem horas',0,80,20,null,'',1]];
...
const zeroMetrics=[['z','Sem positivas nem horas',0,0,100,0,'',1]];
...
for(const objective of ['satisfaction','positive','playtime']){
  ...
  results.push({objective,weights:result.config.weights,normalization:result.normalization,milliseconds:Math.round(performance.now()-start),selected:result.selected.length,candidates:result.candidates,free:result.fixedCount,best:result.best,initial:result.history[0].best,greedy:result.greedy,stats:result.stats});
}
